...
abstract class PaymentModuleCore extends Module
{
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        $price = Product::getPriceStatic((int) $product['id_product'], false, ($product['id_product_attribute'] ? (int) $product['id_product_attribute'] : null), 6, null, false, true, (float)$product['cart_quantity'], false, (int) $order->id_customer, (int) $order->id_cart, (int) $order->{Configuration::get('PS_TAX_ADDRESS_TYPE')}, $specific_price, true, true, null, true, $product['id_customization']);
                        $price_wt = Product::getPriceStatic((int) $product['id_product'], true, ($product['id_product_attribute'] ? (int) $product['id_product_attribute'] : null), 2, null, false, true, (float)$product['cart_quantity'], false, (int) $order->id_customer, (int) $order->id_cart, (int) $order->{Configuration::get('PS_TAX_ADDRESS_TYPE')}, $specific_price, true, true, null, true, $product['id_customization']);
                        ...
                        $product_var_tpl = array(
                            ...
                            'price' => Tools::displayPrice($product_price * (float)$product['quantity'], $this->context->currency, false),
                        ...
                        if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']])) {
                            ...
                            foreach ($customized_datas[$product['id_product']][$product['id_product_attribute']][$order->id_address_delivery] as $customization) {
                                ...
                                $customization_quantity = (float) $customization['quantity'];
                    ...
                    foreach ($this->context->cart->getProducts() as $product) {
                        if ($order_status->logable) {
                            ProductSale::addProductSale((int) $product['id_product'], (float) $product['cart_quantity']);
                        }
                    }
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
						...
						$sel_carrier = $this->context->cart->getSelectedCarrier();
                        ...
                        $data = array(
                            ...
                            '{carrier}' => $sel_carrier['name'],//($virtual_product || !isset($carrier->name)) ? $this->trans('No carrier', array(), 'Admin.Payment.Notification') : $carrier->name,
    ...
    protected function createOrderFromCart(
        ...
        $order->total_shipping_tax_excl = (float) $cart->getTotalShippingCost(null, false);//getPackageShippingCost($carrierId, false, null, $order->product_list);
        $order->total_shipping_tax_incl = (float) $cart->getTotalShippingCost(null, true);//getPackageShippingCost($carrierId, true, null, $order->product_list);
